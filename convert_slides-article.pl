...
@preamble = ('\documentclass[10pt]{paper}', '\usepackage[utf8]{inputenc}', '\usepackage{amsmath}', '\usepackage{url}','\usepackage{graphicx}','\usepackage[margin=2cm]{geometry}');
$theme = 'Warsaw';
%info = ();
@document = ('\begin{document}');
%allowed_keywords = map{$_=>1}('Title', 'Author', 'Date');
$before_document = 1;
$in_frame = 0;
@tabs = ();
@list_type = ();
...
while(<>) {
    ...
    if($before_document == 1) {
        ...
        if(/^(\S+):\s*(\[[^\]]*\])?\s*(.+?)\s*$/) {
            ...
            } elsif($1 eq "Outline" or $1 eq "Institute") {
